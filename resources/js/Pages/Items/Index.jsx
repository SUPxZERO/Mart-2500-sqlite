    ...
    return (
        <POSLayout backgroundImage="/images/backgrounds/bg_items.png">
            ...
            <div className="flex-1 flex flex-col h-full bg-white/40 backdrop-blur-md relative overflow-hidden rounded-2xl border border-slate-200/50 shadow-sm m-4">
                <div className="px-4 sm:px-8 py-4 sm:py-6 bg-white/60 border-b border-slate-200/60 shrink-0 shadow-sm z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 backdrop-blur-md">
                    <div>
                    ...
                    <div className="flex items-center gap-3 w-full md:w-auto">
                        ...
                        <Link
                            href="/items/create"
                            className="shrink-0 flex items-center gap-2 px-4 sm:px-5 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-xl shadow-sm transition-colors"
                        >
                    ...
                    <div className="bg-white/70 backdrop-blur-md rounded-2xl shadow-sm border border-white/50 overflow-hidden">
                        ...
                        <div className="md:hidden divide-y divide-slate-100">
                            {items.data.map(item => {
                                ...
                                return (
                                    <div key={item.id} className={`p-4 flex flex-col gap-3 ${!item.is_active ? 'opacity-50' : ''}`}>
                                        <div className="flex items-start justify-between gap-3">
                                            <div className="flex items-center gap-3">
                                                {item.image_url && !imageErrors[item.id] ? (
                                                    <img src={item.image_url} alt={item.name} className="w-12 h-12 rounded-lg object-cover border border-slate-100 shrink-0" onError={() => handleImageError(item.id)} />
                                                ) : (
                                                    <div className="w-12 h-12 rounded-lg bg-linear-to-br from-slate-100 to-slate-200 flex items-center justify-center shrink-0 text-xl">
                                                        📦
                                                    </div>
                                                )}
                                                <div>
                                                    <h3 className="font-bold text-slate-800">{item.name}</h3>
                                                    <span className="inline-flex items-center gap-1 text-xs text-slate-500 bg-slate-100 px-2 py-0.5 rounded-full mt-1">
                                                        <Tag className="w-3 h-3" /> {item.category}
                                                    </span>
                                                </div>
                                            </div>
                                            <div className="text-right">
                                                <p className="text-emerald-600 font-black">{formatMoney(item.default_price)} <span className="text-xs font-bold text-slate-400">KHR</span></p>
                                                {margin !== null && (
                                                    <p className={`text-xs font-bold mt-0.5 ${margin >= 30 ? 'text-emerald-600' : margin >= 10 ? 'text-amber-500' : 'text-rose-500'}`}>
                                                        {margin}% margin
                                                    </p>
                                                )}
                                            </div>
                                        ...
                                        <div className="flex items-center justify-between mt-2 pt-3 border-t border-slate-50">
                                            <button 
                                                ...
                                                className={`flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-full ${item.is_active ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-500'}`}
                                            >
                                                {item.is_active ? <CheckCircle2 className="w-4 h-4" /> : <ToggleLeft className="w-4 h-4" />}
                                                {item.is_active ? 'Active' : 'Inactive'}
                                            </button>
                                            <Link
                                                href={`/items/${item.id}/edit`}
                                                className="flex items-center gap-1.5 text-xs font-bold text-indigo-600 bg-indigo-50 px-4 py-1.5 rounded-full hover:bg-indigo-100 transition-colors"
                                            >
